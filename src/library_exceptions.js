...
var LibraryExceptions = {
  ...
  $ExceptionInfo__deps: ['__cxa_is_pointer_type', '$ptrToString'],
  $ExceptionInfo: function(excPtr) {
    ...
    this.get_adjusted_ptr_addr = function() {
      return this.ptr + {{{ C_STRUCTS.__cxa_exception.adjustedPtr }}};
    }
  ...
  __cxa_find_matching_catch: function() {
    ...
    for (var i = 0; i < typeArray.length; i++) {
      ...
      if ({{{ exportedAsmFunc('___cxa_can_catch') }}}(caughtType, thrownType, info.get_adjusted_ptr_addr())) {
#if EXCEPTION_DEBUG
